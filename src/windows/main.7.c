...
void native_export_chatlog_init(uint32_t friend_number) {
    ...
    if (GetSaveFileName(&ofn)) {
        ...
        LOG_ERR("Unable to open file and export chatlog.");
    }
}
...
void native_select_dir_ft(uint32_t fid, uint32_t num, FILE_TRANSFER *file) {
    ...
    if (GetSaveFileName(&ofn)) {
        ...
        LOG_ERR("Unable to Get save file for incoming FT.");
    }
}
...
void launch_at_startup(bool should) {
    ...
    if (should) {
        ...
        if (RegOpenKeyW(HKEY_CURRENT_USER, run_key_path, &hKey) == ERROR_SUCCESS) {
            ...
            // 2 bytes per wchar_t
            uint16_t ret = RegSetKeyValueW(hKey, NULL, (LPCSTR)(L"uTox"), REG_SZ, path, path_length * 2);
            if (ret != ERROR_SUCCESS) {
                LOG_ERR("Unable to set Registry key for startup.");
            }
        ...
        if (ERROR_SUCCESS == RegOpenKeyW(HKEY_CURRENT_USER, run_key_path, &hKey)) {
            ...
            if (ret == ERROR_SUCCESS) {
                LOG_ERR("Unable to delete Registry key for startup.");
            }
            RegCloseKey(hKey);
        }
    }
