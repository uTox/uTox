...
void audio_detect(void) {
    ...
    LOG_TRACE("Audio out devices %u" , count);
    ...
    if (pwfx->nSamplesPerSec != 48000 || pwfx->nChannels != 2 || pwfx->wFormatTag != WAVE_FORMAT_EXTENSIBLE) {
        LOG_TRACE("Audio - unsupported format for loopback" );
        goto Exit;
    }
    ...
    LOG_TRACE("Audio frame count %u && Samples/s %lu" , bufferFrameCount, pwfx->nSamplesPerSec);
