...
void drawalpha(int bm, int x, int y, int width, int height, uint32_t color) {
    ...
    uint32_t *out_pixel;
    HBITMAP   temp = CreateDIBSection(curr_win->mem_DC, &bmi, DIB_RGB_COLORS, (void **)&out_pixel, NULL, 0);
    SelectObject(curr_win->mem_DC, temp);
    ...
    AlphaBlend(curr_win->draw_DC, x, y, width, height, curr_win->mem_DC, 0, 0, width, height, blend_function);
...
void draw_image(const NATIVE_IMAGE *image, int x, int y, uint32_t width, uint32_t height, uint32_t imgx, uint32_t imgy) {
    ...
    if (!image_is_stretched(image)) {
        ...
        SelectObject(curr_win->mem_DC, image->bitmap);
        drawdc = curr_win->mem_DC;
        ...
        tmp = CreateCompatibleBitmap(curr_win->mem_DC, image->scaled_width, image->scaled_height);
        ...
        if (image->has_alpha) {
            AlphaBlend(drawdc, 0, 0, image->scaled_width, image->scaled_height, curr_win->mem_DC, 0, 0, image->width,
                       image->height, blend_function);
        } else {
            StretchBlt(drawdc, 0, 0, image->scaled_width, image->scaled_height, curr_win->mem_DC, 0, 0, image->width,
    ...
    // clip and draw
    if (image->has_alpha) {
        AlphaBlend(curr_win->draw_DC, x, y, width, height, drawdc, imgx, imgy, width, height, blend_function);
    } else {
        BitBlt(curr_win->draw_DC, x, y, width, height, drawdc, imgx, imgy, SRCCOPY);
    }
...
void draw_inline_image(uint8_t *img_data, size_t UNUSED(size), uint16_t w, uint16_t h, int x, int y) {
    ...
    SetDIBitsToDevice(curr_win->draw_DC, x, y, w, h, 0, 0, 0, h, img_data, &bmi, DIB_RGB_COLORS);
}
...
void drawtext(int x, int y, const char *str, uint16_t length) {
    ...
    TextOutW(curr_win->draw_DC, x, y, out, length);
}
...
int drawtext_getwidth(int x, int y, const char *str, uint16_t length) {
    ...
    SIZE size;
    TextOutW(curr_win->draw_DC, x, y, out, length);
    GetTextExtentPoint32W(curr_win->draw_DC, out, length, &size);
    return size.cx;
}
...
void drawtextwidth(int x, int width, int y, const char *str, uint16_t length) {
    ...
    DrawTextW(curr_win->draw_DC, out, length, &r, DT_SINGLELINE | DT_END_ELLIPSIS | DT_NOPREFIX);
}
...
void drawtextwidth_right(int x, int width, int y, const char *str, uint16_t length) {
    ...
    RECT r = { x, y, x + width, y + 256 };
    DrawTextW(curr_win->draw_DC, out, length, &r, DT_SINGLELINE | DT_END_ELLIPSIS | DT_NOPREFIX | DT_RIGHT);
}
...
void drawtextrangecut(int x, int x2, int y, const char *str, uint16_t length) {
    ...
    RECT r = { x, y, x2, y + 256 };
    DrawTextW(curr_win->draw_DC, out, length, &r, DT_SINGLELINE | DT_NOPREFIX);
}
...
int textfit(const char *str, uint16_t len, int width) {
    ...
    int  fit;
    SIZE size;
    GetTextExtentExPointW(curr_win->draw_DC, out, length, width, &fit, NULL, &size);
...
void draw_rect_frame(int x, int y, int width, int height, uint32_t color) {
    ...
    SetDCBrushColor(curr_win->draw_DC, color);
    FrameRect(curr_win->draw_DC, &r, hdc_brush);
}

void draw_rect_fill(int x, int y, int width, int height, uint32_t color) {
    RECT r = { x, y, x + width, y + height };
    SetDCBrushColor(curr_win->draw_DC, color);
    FillRect(curr_win->draw_DC, &r, hdc_brush);
}
...
void setfont(int id) {
    SelectObject(curr_win->draw_DC, font[id]);
}

uint32_t setcolor(uint32_t color) {
    return SetTextColor(curr_win->draw_DC, color);
}
...
void pushclip(int left, int top, int width, int height) {
    ...
    SelectClipRgn(curr_win->draw_DC, rgn);
    DeleteObject(rgn);
}
...
void popclip(void) {
    clipk--;
    if (!clipk) {
        SelectClipRgn(curr_win->draw_DC, NULL);
        return;
    }
...
void enddraw(int x, int y, int width, int height) {
    SelectObject(curr_win->window_DC, curr_win->draw_BM);
    BitBlt(curr_win->window_DC, x, y, width, height, curr_win->draw_DC, x, y, SRCCOPY);
}
