...
HRESULT __stdcall dnd_Drop(IDropTarget *UNUSED(lpMyObj), IDataObject *pDataObject,
    ...
    *pdwEffect = DROPEFFECT_COPY;
    ...
    if (r == S_OK) {
        ...
        LOG_INFO("%u files dropped\n", count);

        for (int i = 0; i < count; i++) {
            LOG_NOTE("Sending file number %i", i);
            UTOX_MSG_FT *msg = calloc(1, sizeof(UTOX_MSG_FT));
            if (!msg) {
                LOG_ERR("Unable to alloc for UTOX_MSG_FT");
                return 0;
            }
            ...
            if (!msg->file) {
                LOG_ERR("Unable to read file %s" , path);
                free(msg);
                free(path);
                return 0;
            }
            ...
            LOG_INFO("File number %i sent!" , i);
        }

        ReleaseStgMedium(&medium);
    } else {
        LOG_ERR("itz failed! %lX", r);
    }
