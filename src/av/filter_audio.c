...
bool filter_audio_check(void) {
    ...
    } else if (f_a && !settings.audio_filtering_enabled) { //no return is needed for this one because its already false
        kill_filter_audio(f_a);
        f_a = NULL;
        LOG_INFO("Filter Audio", "filter audio off" );
    }
    return settings.audio_filtering_enabled; //if there is no change return the current value
#else
    return false;
#endif
}
