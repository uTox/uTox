...
void utox_audio_in_listen(void) {
    ...
    if (audio_in_handle) {
        ...
        if (audio_in_handle) {
            ...
            LOG_TRACE("Unable to listen to device!" );
        }
    }
...
void utox_audio_in_device_set(ALCdevice *new_device) {
    if (new_device) {
        ...
        LOG_TRACE("Audio in device changed." );
    } else {
        audio_in_device = NULL;
        audio_in_handle = NULL;
        LOG_TRACE("Audio out device set to null." );
    }
}
...
void utox_audio_out_device_open(void) {
    ...
    if (!audio_out_handle) {
        LOG_TRACE("alcOpenDevice() failed" );
        speakers_on = false;
        return;
    }
    ...
    if (!alcMakeContextCurrent(context)) {
        LOG_TRACE("alcMakeContextCurrent() failed" );
    ...
    if ((error = alGetError()) != AL_NO_ERROR) {
        LOG_TRACE("Error generating source with err %x" , error);
        return;
    }
...
void utox_audio_out_device_set(ALCdevice *new_device) {
    if (new_device) {
        ...
        LOG_TRACE("Audio out device changed." );
    } else {
        audio_out_device = NULL;
        audio_out_handle = NULL;
        LOG_TRACE("Audio in device set to null." );
    }
}
...
void sourceplaybuffer(unsigned int f, const int16_t *data, int samples, uint8_t channels, unsigned int sample_rate) {
    ...
    // LOG_TRACE("audio frame || samples == %i channels == %u rate == %u " , samples, channels, sample_rate);

    ALint state;
    alGetSourcei(source, AL_SOURCE_STATE, &state);
    if (state != AL_PLAYING) {
        alSourcePlay(source);
        // LOG_TRACE("Starting source %u" , i);
    }
}
...
static void audio_in_init(void) {
    ...
    if (audio_in_device_list) {
        ...
        LOG_TRACE("input device list:" );
        while (*audio_in_device_list) {
            LOG_TRACE("\t%s" , audio_in_device_list);
...
static void audio_out_init(void) {
    ...
    if (audio_out_device_list) {
        ...
        LOG_TRACE("output device list:" );
        while (*audio_out_device_list) {
            LOG_TRACE("\t%s" , audio_out_device_list);
...
static void generate_melody(MELODY melody[], uint32_t seconds, uint32_t notes_per_sec, ALuint *target) {
    ...
    if ((error = alGetError()) != AL_NO_ERROR) {
        LOG_TRACE("Error generating buffer with err %i" , error);
        return;
    }
    ...
    if (!samples) {
        LOG_TRACE("Unable to generate ringtone buffer!" );
        return;
    }
...
void utox_audio_thread(void *args) {
    ...
    LOG_TRACE("frame size: %u" , perframe);
    ...
    if (!preview_buffer) {
        LOG_ERR("Unable to allocate memory for preview buffer.");
        return;
    }
    ...
    while (1) {
        ...
        if (audio_thread_msg) {
            ...
            switch (m->msg) {
                ...
                case UTOXAUDIO_PLAY_RINGTONE: {
                    if (settings.ringtone_enabled && self.status != USER_STATUS_DO_NOT_DISTURB) {
                        LOG_INFO("Going to start ringtone!" );
                ...
                case UTOXAUDIO_STOP_RINGTONE: {
                    LOG_INFO("Going to stop ringtone!" );
                ...
                case UTOXAUDIO_PLAY_NOTIFICATION: {
                    if (settings.ringtone_enabled && self.status == USER_STATUS_AVAILABLE) {
                        LOG_INFO("Going to start notification tone!" );
            ...
            if (close_device_in && close_device_in <= currtime) {
                LOG_INFO("close device triggered!" );
                utox_audio_out_device_close();
                close_device_in = 0;
            }
        }
        ...
        if (!f_a && settings.audiofilter_enabled) {
            ...
            if (!f_a) {
                ...
                LOG_TRACE("filter audio failed" );
            } else {
                LOG_TRACE("filter audio on" );
            }
        } else if (f_a && !settings.audiofilter_enabled) {
            kill_filter_audio(f_a);
            f_a = NULL;
            LOG_TRACE("filter audio off" );
        }
        #else
                ...
                if (voice) {
                    ...
                    for (size_t i = 0; i < self.friend_list_count; i++) {
                        if (UTOX_SEND_AUDIO(i)) {
                            ...
                            if (error) {
                                LOG_TRACE("toxav_send_audio error friend == %lu, error ==  %i" , i, error);
                            } else {
                                // LOG_TRACE("Send a frame to friend %i" ,i);
                                if (active_call_count >= UTOX_MAX_CALLS) {
                                    LOG_TRACE("We're calling more peers than allowed by UTOX_MAX_CALLS, This is a bug" );
                                    break;
                                }
                            }
                        }
                                    ...
                                    uint8_t channels, unsigned int sample_rate, void *userdata)
{
    ...
    if(processed) {
        ...
        LOG_TRACE("dropped audio frame %i %i" , groupnumber, peernumber);
        return;
    }
    ...
    if(state != AL_PLAYING) {
        ...
        LOG_TRACE("Starting source %i %i" , groupnumber, peernumber);
    }
}
