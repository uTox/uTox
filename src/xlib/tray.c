...
static void tray_reposition(void) {
    LOG_NOTE("Reposition Tray");

    uint32_t null;
    XGetGeometry(display, tray_window.window, &root_window, &tray_window._.x, &tray_window._.y,
                &tray_window._.w, &tray_window._.h,
                &null, &null);
    LOG_NOTE("New geometry x %u y %u w %u h %u", tray_window._.x, tray_window._.y, tray_window._.w, tray_window._.h);

    LOG_INFO("Setting to square");
...
static void draw_tray_icon(void) {
    ...
    if (NATIVE_IMAGE_IS_VALID(icon)) {
        ...
        LOG_ERR("Tray no workie, that not gud!");
    }
}

static void tray_xembed(XClientMessageEvent *ev) {
    LOG_NOTE("ClientMessage on display %u", ev->display);
    LOG_NOTE("Format (%i) as long %lu %lu parent window %lu proto version %lu %lu",
...
void create_tray_icon(void) {
    LOG_NOTE("Create Tray Icon");

    LOG_NOTE("Resolution %u %u", tray_window._.w, tray_window._.h);
...
bool tray_window_event(XEvent event) {
    if (event.xany.window != tray_window.window) {
        LOG_ERR("in %u oun %u", event.xany.window, tray_window.window);
        return false;
    }
    ...
    switch (event.type) {
        ...
        case EnterNotify: {
            LOG_INFO("EnterNotify");
            return true;
        }
        case LeaveNotify: {
            LOG_INFO("LeaveNotify");
            return true;
        }

        case ReparentNotify: {
            LOG_WARN("ReparentNotify");
            return true;
        }

        default: {
            LOG_ERR("Incoming tray window event (%u)", event.type);
            break;
        }
    }
    LOG_ERR("Reached end of function, this is bad juju!");
    return false;
}
