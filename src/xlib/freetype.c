...
GLYPH *font_getglyph(FONT *f, uint32_t ch) {
    ...
    // LOG_TRACE("%u %u %u %u %C" , PIXELS(i->face->size->metrics.height), g->width, g->height, p->bitmap.pitch, ch);
...
static void font_info_open(FONT_INFO *i, FcPattern *pattern) {
    ...
    if (ft_error != 0) {
        LOG_TRACE("Freetype error %u %s %i" , ft_error, filename, id);
        return;
    }

    ft_error = FT_Set_Char_Size(i->face, (size * 64.0 + 0.5), (size * 64.0 + 0.5), 0, 0);
    if (ft_error != 0) {
        LOG_TRACE("Freetype error %u %lf" , ft_error, size);
        return;
    }

    // LOG_TRACE("Loaded font %s %u %i %i" , filename, id, PIXELS(i->face->ascender), PIXELS(i->face->descender));
}
