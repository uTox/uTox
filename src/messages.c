...
static int msgheight(MSG_HEADER *msg, int width) {
    switch (msg->msg_type) {
        case MSG_TYPE_NULL: {
            LOG_ERR("Invalid message type in msgheight.");
            return 0;
        }
...
static int msgheight_group(MSG_HEADER *msg, int width) {
    switch (msg->msg_type) {
        ...
        default: {
            LOG_TRACE("Error, can't set this group message height" );
        }
    }
...
static bool msg_add_day_notice(MESSAGES *m, time_t last, time_t next) {
        ...
        || (ltime_year == msg_time->tm_year && ltime_mon == msg_time->tm_mon && ltime_day < msg_time->tm_mday))
    {
        ...
        if (!msg) {
            LOG_ERR("Couldn't allocate memory for day notice.");
            return false;
        }
...
uint32_t message_add_type_notice(MESSAGES *m, const char *msgtxt, uint16_t length, bool log) {
    MSG_HEADER *msg = calloc(1, sizeof(MSG_HEADER));
    if (!msg) {
        LOG_ERR("Couldn't allocate memory for notice.");
        return UINT32_MAX;
    }
...
bool message_log_to_disk(MESSAGES *m, MSG_HEADER *msg) {
    ...
    if (!f) {
        LOG_ERR("Could not get friend with number: %u", m->id);
        return false;
    }
    ...
    switch (msg->msg_type) {
        ...
        default: {
            LOG_NOTE("uTox Logging:\tUnsupported message type %i", msg->msg_type);
        }
    }
    return false;
}
...
bool messages_read_from_log(uint32_t friend_number) {
    ...
    if (data) {
        ...
        LOG_ERR("uTox Logging:\tFound chat log entries, but couldn't get any data. This is a problem.");
    }
...
void messages_clear_receipt(MESSAGES *m, uint32_t receipt_number) {
        ...
        if (m->data[start]) {
            ...
            if (msg->msg_type == MSG_TYPE_TEXT || msg->msg_type == MSG_TYPE_ACTION_TEXT) {
                if (msg->receipt == receipt_number) {
                    ...
                    if (msg->disk_offset) {
                        LOG_TRACE("Updating message -> disk_offset is %lu" , msg->disk_offset);
                        ...
                        LOG_TRACE("Updating first message -> disk_offset is %lu" , msg->disk_offset);
                        utox_update_chatlog(hex, msg->disk_offset, data, length);
                    } else {
                        LOG_ERR("Messages:\tUnable to update this message...\n"
    ...
    LOG_ERR("Received a receipt for a message we don't have a record of. %u" , receipt_number);
    pthread_mutex_unlock(&messages_lock);
}
                              ...
                              int x, int y, int w, int UNUSED(h))
{
    ...
    if (ny < y || (uint32_t)(ny - y) + MESSAGES_SPACING != msg_height) {
        LOG_TRACE("Text Draw Error:\ty %i | ny %i | mheight %u | width %i " , y, ny, msg_height, w);
    }
...
void messages_draw(PANEL *panel, int x, int y, int width, int height) {
    ...
    for (size_t curr_msg_i = 0; curr_msg_i != n; curr_msg_i++) {
        ...
        if (m->is_groupchat) {
            ...
            switch (msg->msg_type) {
                case MSG_TYPE_NULL: {
                    ...
                    LOG_ERR("Invalid message type in messages_draw.");
                    break;
                }
        ...
        switch (msg->msg_type) {
            case MSG_TYPE_NULL: {
                LOG_ERR("Error msg type is null");
                break;
            }
                    ...
                    int UNUSED(dy))
{
    ...
    while (i < m->number) {
        ...
        if (my >= 0 && my < dy) {
            ...
            switch (msg->msg_type) {
                case MSG_TYPE_NULL: {
                    LOG_ERR("Invalid message type in messages_mmove.");
                    return false;
                }
...
bool messages_mdown(PANEL *panel) {
    ...
    if (m->cursor_over_msg != UINT32_MAX) {
        ...
        switch (msg->msg_type) {
            case MSG_TYPE_NULL: {
                LOG_ERR("Invalid message type in messages_mdown.");
                return false;
            }
            ...
            case MSG_TYPE_NOTICE_DAY_CHANGE: {
                if (m->cursor_over_uri != UINT32_MAX) {
                    ...
                    LOG_TRACE("mdn dURI %u, oURI %u" , m->cursor_down_uri, m->cursor_over_uri);
                }
...
bool messages_dclick(PANEL *panel, bool triclick) {
    ...
    if (m->cursor_over_msg != UINT32_MAX) {
        ...
        switch (msg->msg_type) {
            case MSG_TYPE_NULL: {
                LOG_ERR("Invalid message type in messages_dclick.");
                return false;
            }
    ...
    if (m->cursor_over_msg != UINT32_MAX) {
        ...
        if (msg->msg_type == MSG_TYPE_TEXT) {
            if (m->cursor_over_uri != UINT32_MAX && m->cursor_down_uri == m->cursor_over_uri
                ...
                && !m->selecting_text) {
                LOG_TRACE("mup dURI %u, oURI %u" , m->cursor_down_uri, m->cursor_over_uri);
                char url[m->urllen + 1];
...
int messages_selection(PANEL *panel, char *buffer, uint32_t len, bool names) {
    ...
    while (i != UINT32_MAX && i != n) {
        ...
        switch (msg->msg_type) {
            case MSG_TYPE_NULL: {
                LOG_ERR("Invalid message type in messages_selection.");
                return 0;
            }
...
bool messages_char(uint32_t ch) {
    ...
    if (flist_get_friend()) {
        ...
        LOG_TRACE("Can't type to nowhere");
        return false;
    }
...
void message_free(MSG_HEADER *msg) {
    ...
    switch (msg->msg_type) {
        case MSG_TYPE_NULL: {
            LOG_ERR("Invalid message type in message_free.");
            break;
        }
