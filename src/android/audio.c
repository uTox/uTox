            ...
            for (i = 0; i < MAX_CALLS; i++) {
                if (call[i]) {
                    ...
                    /*if((r = toxav_prepare_audio_frame(arg, i, dest, sizeof(dest), frame, 960)) < 0) {
                        LOG_TRACE(__FILE__, "toxav_prepare_audio_frame error %i" , r);
                        continue;
                    }

                    if((r = toxav_send_audio(arg, i, dest, r)) < 0) {
                        LOG_TRACE(__FILE__, "toxav_send_audio error %i %s" , r, strerror(errno));
                    }*/
