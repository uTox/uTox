...
static int64_t parseargument(uint8_t *dest, char *src, size_t length, void **pdns3) {
    ...
    *d = 0;
    ...
    LOG_TRACE("Parsed: (%.*s)->(%s) pin=%X" , (int)length, src, dest, pin);
...
// TODO: This should be moved to a native file.
    ...
    *p++ = 0;
    ...
    LOG_TRACE("%s reponded %u" , value, len);

    p = memmem(packet, len, "v=tox", 5);
    if (p) {
        LOG_TRACE("test %u" , *(p - 1));
        p[*(p - 1)] = 0;
    ...
    if ((len = res_query((char *)result, C_IN, T_TXT, answer, PACKETSZ)) >= 0) {
        ...
        if ((len = dn_expand(answer, answend, pt, host, sizeof(host))) < 0) {
            LOG_TRACE("^dn_expand failed" );
            goto FAIL;
        }

        pt += len;
        if (pt > answend - 4) {
            LOG_TRACE("^Bad (too short) DNS reply" );
            goto FAIL;
        }

        GETSHORT(type, pt);
        if (type != T_TXT) {
            LOG_TRACE("^Broken DNS reply." );
            goto FAIL;
        }
        ...
        do { /* recurse through CNAME rr's */
            pt += size;
            if ((len = dn_expand(answer, answend, pt, host, sizeof(host))) < 0) {
                LOG_TRACE("^second dn_expand failed" );
                goto FAIL;
            }
            ...
            if (pt + size < answer || pt + size > answend) {
                LOG_TRACE("^DNS rr overflow" );
                goto FAIL;
            }
        } while (type == T_CNAME);

        if (type != T_TXT) {
            LOG_TRACE("^Not a TXT record" );
            goto FAIL;
        }

        if (!size || (txtlen = *pt) >= size || !txtlen) {
            LOG_TRACE("^Broken TXT record (txtlen = %d, size = %d)" , txtlen, size);
            goto FAIL;
        }
        ...
        // answer[len] = 0;
        // LOG_TRACE("%u %u" , txtlen, answend - (pt + 1));
        LOG_TRACE("Attempting:\n%.*s" , txtlen, pt + 1);
...
void dns_request(char *name, size_t length) {
    if (settings.force_proxy) {
        LOG_TRACE("uTox DNS:\tUnable to do DNS lookup, because we're are using a proxy without UDP!" );
        return;
    }
