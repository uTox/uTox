...
static void draw_settings_header(int UNUSED(x), int UNUSED(y), int UNUSED(width), int UNUSED(height)){
    ...
    #ifdef GIT_VERSION
        setfont(FONT_TEXT);
        drawtext(LIST_RIGHT + UTOX_STR_WIDTH(UTOX_SETTINGS), SCALE * 5, (uint8_t*)GIT_VERSION, strlen(GIT_VERSION));
    #endif
}
...
static void draw_settings_text_ui(int x, int y, int w, int UNUSED(height)){
    ...
    drawstr(LIST_RIGHT + 10 * SCALE + UTOX_STR_WIDTH(CLOSE_TO_TRAY), y + 55 * SCALE, START_IN_TRAY);
