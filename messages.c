        ...
        case MSG_TYPE_FILE: {
            ...
            if(file->status == FILE_DONE) {
                ...
                if(file->status == FILE_OK) {
                    ...
                    drawtext_common(target, xx + 5 * SCALE + 53 * SCALE - textwidth_common(target, text, len) / 2, y + 10 * SCALE, text, len);
                    ...
                    drawtext_common(target, xx + 5 * SCALE + 106 * SCALE - textwidth_common(target, text, len), y + 10 * SCALE, text, len);
                }
            }


            drawtextwidth_common(target, xx + 5 * SCALE, 106 * SCALE, y + 3 * SCALE, file->name, file->name_length);
            drawtext_common(target, xx + 5 * SCALE, y + 10 * SCALE, size, sizelen);
